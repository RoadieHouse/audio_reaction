...
Let users attach a diagram to a session — drawn directly in the app or uploaded as an image (e.g. [flutter_drawing_board](https://pub.dev/packages/)). Example use case: a half-circle of cones where each cone is labelled with a number or colour, and each label maps to a specific audio cue. The athlete sees the layout once before the session starts, then reacts purely by sound during training.
